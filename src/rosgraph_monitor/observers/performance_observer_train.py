...
class PerformanceObserverTrain(TopicObserver):
    ...
    def calculate_attr(self, msgs):
        ...
        for idx in range(len(path_poses)):
            path_pose_distance = sqrt((path_poses[idx].pose.position.x - x_robot)**2 + (path_poses[idx].pose.position.y-y_robot)**2)
            if path_pose_distance < path_distance:
                path_distance = path_pose_distance
                x_path = path_poses[idx].pose.position.x
                y_path = path_poses[idx].pose.position.y
            if path_pose_distance > (path_distance + 1.0):
